...
watch(
    () => form,
    throttle(() => {
        router.get(urls.printers.index(), pickBy(form), { preserveState: true })
    }, 150),
    { deep: true }
)
            ...
            <Column header="Расходные материалы">
                <template #body="{ data }">
                    ...
                    <div v-if="showConsumables[data.id]" class="grid gap-y-2 divide-y divide-slate-300 divide-dashed text-sm mt-4">
                        <div class="flex flex-row gap-1 pt-1" v-for="consumable in data?.printer?.consumables_deep" :key="consumable.id">
